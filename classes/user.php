...
class User
{
	...
	function create_post($texts, $dates, $user)
	{
		...
		if (empty($text) && empty($_FILES['image']['name']) && empty($_FILES['video']['name'])) {
	...
	function select_post()
	{
		...
		if ($select->rowCount() >= 1) {
			...
			foreach ($row as $val) {
?>
				<form method="post" action="post.php" style="background: white;border: 0px;">
					<fieldset style="background-color: white;text-align: center;width: 80%;margin-left: 2cm;">
						...
						if ($select->rowCount() >= 1) {
							...
							if ($row->status == 'online') { ?>
								<div class="active-user-wrapper w3-round-large" id="">
									<div class="active-user-data-wrapper">
										<div class="active-user-image-wrapper w3-white w3-circle"><img src='../images/<?php echo $row->profile_pic; ?>' class="w3-image w3-circle" /><span class="online-dot"></span></div><span class="name"></span>
									</div>
								</div>
							<?php } else {
							?>
								<div class="active-user-wrapper w3-round-large" id="">
									<div class="active-user-data-wrapper">
										<div class="active-user-image-wrapper w3-white w3-circle"><img src='../images/<?php echo $row->profile_pic; ?>' class="w3-image w3-circle" /></div><span class="name"></span>
									</div>
								</div>
							<?php }
						}
						...
						if (empty($val->image) && empty($val->video)) {
							...
							echo "<h4 class='postTitle'>{$val->post}</h4><br>";
						...
						<?php } else if (empty($val->image) && !empty($val->video)) {
							echo "<div class='username'><a href='../friends/profile1.php?id=$id' style='color:green;font-size:30px'> " . $val->username . "</a><i style='color: gray;'> on " . $dates . "</i></div><br>";
							echo "<h4 class='postTitle'>{$val->post}</h4><br>";
						?><video src="videos/<?php echo $val->video; ?>" style="width: 100%;max-height: 40vh;" controls>
								<!-- <source src="videos/<?php echo $val->video; ?>" type="video/mp4"> -->
							</video>
						<?php }
						?>
						<br>
